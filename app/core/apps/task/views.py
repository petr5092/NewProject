...
from core.apps.task.models import (
    Task,
    UserTaskLike,
    UserBookmarks,
    BrowHistory,
...
from core.apps.task.forms import (
    TaskCreate,
    CategoryCreate,
    UserCreateForm,
    UserAuthForm,
...
class GetHistory(TaskListView):
    def get_queryset(self) -> QuerySet[Task]:
        if isinstance(self.request.user, AnonymousUser):
            return render(
                self.request, "task/auth_user.html", context={"form": UserAuthForm}
            )
        return (
            super()
            .get_queryset()
            .filter(bhistory__user=self.request.user)
            .prefetch_related("bmarks", "like")
            .select_related("category")
            .all()
        )
...
class FilterBmarks(TaskListView):
    def get_queryset(self) -> QuerySet[Task]:
        ...
        return (
            super()
            .get_queryset()
            .filter(bmarks__user=self.request.user)
            .prefetch_related("bmarks", "like")
            .select_related("category")
            .all()
        )
